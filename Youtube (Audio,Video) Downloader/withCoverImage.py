...
def download_youtube(url, media_type):
    ...
    ydl_opts = {}
    if media_type == "1":  # Video with audio
        ydl_opts = {
            'format': 'bestvideo[height<=720][ext=mp4]+bestaudio[ext=m4a]/best[ext=mp4]/best',
            'outtmpl': os.path.join(path_video, '%(title)s.%(ext)s'),
            'merge_output_format': 'mp4',
        }
    elif media_type == "2":  # Audio only mp3 with THUMBNAIL
        ydl_opts = {
            'format': 'bestaudio/best',
            'outtmpl': os.path.join(path_audio, '%(title)s.%(ext)s'),
            'writethumbnail': True,  # Downloads the thumbnail image file
            'postprocessors': [
                {
                ...
                {
                    'key': 'EmbedThumbnail',  # Embeds the thumbnail into the MP3 metadata
                },
            ],
        }
    else:
...
def main():
    ...
    if len(sys.argv) > 1 and sys.argv[1] in ['-h', '--help', '-help', 'help', '-?', '-man']:
        print("Usage: python Songs.Downloader.Youtube.py [<YouTube URL> <media type>]")
        return
